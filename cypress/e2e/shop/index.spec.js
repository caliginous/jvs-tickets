    ...
    it("Select Free Seats", () => {
        ...
        cy.fixture("admin/events").then((eventFixture) => {
            ...
            cy.get("#seat-selection-free-total-price").should(
                "have.text",
                formatPrice(firstCategory.price, "GBP")
            );
            ...
            cy.get("#seat-selection-free-total-price").should(
                "have.text",
                formatPrice(secondCategory.price, "GBP")
            );
    ...
    it("Service Fees", () => {
        cy.fixture("admin/user").then((userFixture) => {
            cy.task("getAdminToken").then((token) => {
                ...
                cy.get("#information-address-next").click();
                cy.get("#checkbox-post").should("contain.text", formatPrice(firstCategory.price, "GBP"));
                cy.get("#checkbox-post").click();
                cy.get("#stepper-next-button").click();
                ...
                cy.get("#checkbox-invoice").should("contain.text", formatPrice(secondCategory.price, "GBP"));
                cy.get("#checkbox-invoice").click();

                cy.get(".payment-overview-service-fee").should("have.length", 2);
                cy.get(".payment-overview-service-fee").eq(0).should("contain.text", formatPrice(firstCategory.price, "GBP"));
                cy.get(".payment-overview-service-fee").eq(1).should("contain.text", formatPrice(secondCategory.price, "GBP"));
                ...
                cy.personalInformation();
                cy.get("#information-address-next").click();
                cy.get("#checkbox-post").should("not.contain.text", formatPrice(firstCategory.price, "GBP"));
                cy.get("#checkbox-post").click();
                cy.get("#stepper-next-button").click();

                cy.url().should("include", "payment");

                cy.get("#checkbox-invoice").should("not.contain.text", formatPrice(secondCategory.price, "GBP"));
                cy.get("#checkbox-invoice").click();
                ...
                cy.request(
                  {
                    ...
                    cy.get("[data-testid='accordion-content'] [data-testid='body-text']").should("have.length", 2);
                    cy.get("[data-testid='accordion-content'] [data-testid='body-text']").should((elements) => {
                        const texts = elements.toArray().map(elem => elem.innerText);
