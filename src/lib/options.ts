...
const DEFAULT_OPTIONS: Partial<Record<Options, any>> = {
    ...
    "email.support-email": "support@jvs.org.uk",
    "email.app-name": "JVS Events",
    "email.app-url": "https://jvs.org.uk",
    "email.sender-name": "JVS Events"
}
...
export const setOptionData = async (key: Options, value: Uint8Array, type: string) => {
...
export const getOptionData = async (key: Options, fallback?): Promise<{data: Buffer, type: string}> => {
    ...
    if (!option) return {data: fallback ?? null as any, type: null};
    return {data: Buffer.from(option.data as any), type: JSON.parse(option.value).value};
}
