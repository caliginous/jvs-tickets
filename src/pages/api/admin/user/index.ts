...
export default async function handler(
    ...
    if (req.method === "POST") {
        const { username, email, password, readRights, writeRights } = req.body;
        ...
        let additionalData = {
            readRights: JSON.stringify(readRights || []),
            writeRights: JSON.stringify(writeRights || [])
        };
        if (canRegister) {
        ...
        try {
            console.log(`[admin/user] 📧 Sending welcome email to new admin user: ${user.email}`);
            const { emailTriggerService } = await import('../../../../lib/services/emailTriggerService');
            
            await emailTriggerService.sendWelcomeEmail({
                userEmail: user.email,
                userFirstName: user.userName, // Use username as firstName for admin users
                userLastName: 'Admin', // Default lastName for admin users
                locale: 'en'
            });
            console.log(`[admin/user] ✅ Welcome email sent successfully to: ${user.email}`);
        } catch (emailError) {
            console.error(`[admin/user] ❌ Failed to send welcome email:`, emailError);
