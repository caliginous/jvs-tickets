...
export default async function handler(
    ...
    if (req.method === "PUT") {
        ...
        try {
            // Allow bypassing recaptcha for Stripe flow with special token
            let tokenResult = RecaptchaResultType.Success;
            if (token !== 'manual-reservation') {
                tokenResult = await verifyToken(
            ...
            // For Stripe flow (manual-reservation token), bypass seat validation since we don't have seat IDs yet
            const bypassSeatValidation = token === 'manual-reservation';
            ...
            const [orderValid, invalidTickets] = await validateOrder(tickets, eventDateId, id, true, bypassSeatValidation)
            ...
            if (!orderValid) { // quick check to enhance valid order speed
                ...
                console.log('Filtered validTickets:', validTickets);
            }
